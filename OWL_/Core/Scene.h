...
namespace Core
{
	...
	public:
		Scene(Camera& camera, Graphics::Texture2D& floatBuffer, Graphics::Texture2D& resolvedBuffer, UINT width = 1280, UINT height = 720) :
			m_Camera(camera),
		...
		inline Geometry::Model* GetMirror() { return m_pMirror; }
	...
	public:
		std::vector<Geometry::Model*> pRenderObjects; // opaque.
	...
	private:
		UINT m_ScreenWidth;
		UINT m_ScreenHeight;
		Geometry::Mesh* m_pScreenMesh = nullptr;
		Camera& m_Camera;
		...
		Graphics::Texture2D m_DepthOnlyBuffer; // No MSAA
		...
		std::vector<Geometry::Model*> m_ppLightSpheres;
