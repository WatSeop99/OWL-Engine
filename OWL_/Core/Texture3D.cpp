...
namespace Core
{
	void Texture3D::Initialize(ID3D11Device* pDevice, UINT width, UINT height, UINT depth, DXGI_FORMAT pixelFormat, const std::vector<float>& INIT_DATA)
	{
		m_width = width;
		m_height = height;
		m_depth = depth;
		...
		destroy();
		Graphics::CreateTexture3D(pDevice, width, height, depth, pixelFormat, INIT_DATA, 
	...
	void Texture3D::InitNoiseF16(ID3D11Device* pDevice)
	{
		...
		Initialize(pDevice, width, height, depth, DXGI_FORMAT_R16G16B16A16_FLOAT, f16);
	}
