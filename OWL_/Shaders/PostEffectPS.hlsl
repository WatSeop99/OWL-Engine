...
float HaloEmission(float3 posView, float radius)
{
    ...
    float3 center = mul(float4(lights[1].position, 1.0f), view).xyz; // View 공간으로 변환
...
float4 main(SamplingPixelShaderInput input) : SV_TARGET
{
    if (mode == 1)
    {
        float3 color = clamp(renderTex.Sample(linearClampSampler, input.texcoord).rgb, 0.0f, 1.0f);
